import { Log } from "@ethersproject/abstract-provider";

import * as syncEventsUtils from "@/events-sync/utils";
import * as blocksModel from "@/models/blocks";

export type TransactionStatus = 0 | 1;

export type BaseEventParams = {
  address: string;
  block: number;
  blockHash: string;
  txHash: string;
  txIndex: number;
  txNonce?: number;
  txStatus?: TransactionStatus;
  logIndex: number;
  timestamp: number;
  batchIndex: number;
};

export const parseEvent = async (
  log: Log,
  blocksCache: Map<string, blocksModel.Block>,
  chainId = 1,
  batchIndex = 1
): Promise<BaseEventParams> => {
  const address = log.address.toLowerCase();
  const blockNumber = log.blockNumber;
  const blockHash = log.blockHash.toLowerCase();
  const txHash = log.transactionHash.toLowerCase();
  const txIndex = log.transactionIndex;
  const logIndex = log.logIndex;

  let blockResult = blocksCache.get(`${chainId}-${blockNumber}`);
  if (!blockResult) {
    const block = await syncEventsUtils.fetchBlock(chainId, blockNumber);
    blocksCache.set(`${chainId}-${blockNumber}`, block);
    blockResult = blocksCache.get(`${chainId}-${blockNumber}`)!;
  }

  return {
    address,
    txHash,
    txIndex,
    block: blockNumber,
    blockHash,
    logIndex,
    timestamp: blockResult.timestamp,
    batchIndex,
  };
};
