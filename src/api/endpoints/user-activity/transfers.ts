...
export const getTransferDetails: RouteOptions = {
  ...
  validate: {
    query: Joi.object({
      hash: Joi.string()
        .lowercase()
        .pattern(regex.txhash)
        .description(
          "Filter to a particular hash, e.g. `0x89da45e4f9e20a6cad963c801835305e6d02616847eb2b3391321a8ac97baf03`"
        ),
      limit: Joi.number().integer().min(1).max(100).default(20),
      continuation: Joi.string().pattern(regex.base64),
    }),
  ...
  handler: async (request: Request) => {
    ...
    try {
      // TODO: map result to JOI object
      return UserActivities.getActivityDetails(query.hash);
    } catch (error) {
      logger.error(`get-users-transfers-${version}-handler`, `Handler failure: ${error}`);
      throw error;
    }
